...
from bop_erp.purchasing.exceptions import PurchaseDriftError
...
def check_purchase_operation_replay(
	...
	"""
	Checks if a purchasing operation has already been executed for this operation_key.
	...
	event = frappe.db.sql(
		...
		LIMIT 1
		""",
		(active_key, IntegrationProvider.INTERNAL, operation_key, entity_type),
		as_dict=True,
	)
...
def record_purchase_operation(
	...
	event_doc = frappe.get_doc({
		...
		"request_metadata": json.dumps({"purchase_operation": operation_type}),
